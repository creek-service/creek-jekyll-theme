...
#
# Taken from https://github.com/mslinn/jekyll_flexible_include_plugin under MIT license
...
class FlexibleInclude < Liquid::Tag
  ...
  def render(liquid_context) # rubocop:disable Metrics/AbcSize, Metrics/PerceivedComplexity, Metrics/MethodLength, Metrics/CyclomaticComplexity
    ...
    when /\A\// # Absolute path
      # disable in this copy of the plugin, as not needed
      return denied("Access to the absolute path #{path} is not supported.")
    ...
    when /\A~/ # Relative path to user's home directory
      # disable in this copy of the plugin, as not needed
      return denied("Access to the file under users home directory #{path} is not supported.")
    ...
    when /\A!/ # Run command and return response
      # disable in this copy of the plugin, as not needed
      contents = nil
      return denied("Arbitrary command execution #{path} is not supported.")
  ...
  def denied(msg)
    raise "Error in file #{@helper.page['path']} - #{msg}"
  end
  ...
  def read_file(file)
    begin
      ...
      raise "Error in file #{@helper.page['path']}: #{e.message}"
    end
  end
